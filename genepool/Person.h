...
public:
  // Constructor
  Person();
  Person(std::string name, std::string gender, std::string mother,std::string father);
  // Destructor?
  ...
  void m(Person *);
  void f(Person *);
  std::set<Person*> add(Person * root, std::set<Person*>);
  std::set<Person*> Children;
  std::set<Person*> addD(Person* root, std::set<Person*>);
};
